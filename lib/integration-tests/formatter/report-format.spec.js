...
describe('Report format', () => {
  ...
  describe('tree format', () => {
    ...
    it('should group units by common ancestors', () => {
      const allResults = testResultLogs.join('\n');
      expect(allResults)
      const singleAncestorResult = allResults.slice(0, 2);
      const twoAncestoryResult = allResults.slice(2, 4);

      expect(allResults).toContainEqual('Boolean comparison');

      expect(singleAncestorResult).toNestNWithTotalDepthOf(2, 4);
      expect(twoAncestoryResult).toNestNWithTotalDepthOf(2, 6);
    ...
    const toNestNWithDepthOf = (received, numUnits, indentLen) => {
    ...
    expect.extend({
      toNestWithDepthOf(received, indentLen) {
        return toNestNWithDepthOf(received, 1, indentLen);
      },
      toNestNWithTotalDepthOf(received, numUnits, indentLen) {
        return toNestNWithDepthOf(received, numUnits, indentLen);
      },
    });
  });
